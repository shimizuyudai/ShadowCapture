...
{
    public class DividedTextFileManager
    {
        ...
        //***_info.jsonのpath
        public DividedTextFileManager(string path)
        {
        ...
        public DividedTextFileManager(string path, int maxLineNUM, string prefix, string extension)
        {
        ...
        private void SaveInfo(DivideInfo info)
        {
        ...
        public async Task<string[]> ReadLines(long offset, long num, long step = 1)
        {
            var start = Math.Max(0, offset);
            var end = offset + num * step - 1;
            step = Math.Max(step, 1);

            return await ReadLines(start, end, step);
        }

        //末尾から取得する
        public async Task<string[]> ReadLinesFromEnd(long offset, long num, long step = 1)
        {
            var end = (Info.TotalLineNUM - 1 - offset);
            var start = Math.Max(end - (step * (num - 1)), (end % step));
            var lines = await GetLines(start, end, step);
            return lines.Reverse().ToArray();
        }

        private async Task<string[]> GetLines(long start, long end, long step = 1)
        {
            ...
            await Task.Run(() =>
            {
                ...
                if (info != null)
                {
                    ...
                    if (start >= end) return;
                    var list = new ConcurrentBag<DividedTextFile>();
                    ...
                    Parallel.For(
                        0, fileNUM, i =>
                        {
                            ...
                            {
                                var dividedFile = new DividedTextFile(i);
